 ...
 * 1. 회원가입("/enroll") - get enroll.jsp ,  post 회원가입 DB다녀와서 첫 이동
 * 회원가입화면 - 회원가입(enroll post)버튼
 * 2. 로그인("/login") - Get login.jsp, post 로그인확인 DB다녀와서 session("lgnss")등록(이름, email, id) 첫 이동
 * 3. 로그아웃("/logout") - Get 세션만료 후 첫 이동
 * 4. 내정보 보기("/myinfo") - get myinfo.jsp
 * 내정보 보기 화면에서 하단에 수정(/infoupdate get), 탈퇴 버튼
